    ...
    presetScripts: [
            {
                ...
                description: "拦截并监控所有XMLHttpRequest请求，URL request can 查看、method、content parameters response and。API analysis request call for and 适AJAX。",
...
(function() {
    ...
    XMLHttpRequest.prototype.open = function(method, url, async, user, password) {
        this._method = method;
        this._url = url;
        console.log('[XHR Hook] start request:', method, url);
    ...
    XMLHttpRequest.prototype.send = function(data) {
        this.addEventListener('readystatechange', function() {
            if (this.readyState === 4) {
                console.log('[XHR Hook] complete response:', {
                    method: this._method,
                    url: this._url,
                    status: this.status,
        ...
        if (data) {
            console.log('[XHR Hook] data request:', data);
        }
    ...
    console.log('[XHR Hook] XMLHttpRequest Hook 已激活');
            ...
            {
                name: "Hook Fetch API",
                description: "request monitor intercept all 并fetch，information response request display of and 详细。现代浏览器in替代XMLHttpRequest的新API监控。",
                code: `// Hook Fetch API
(function() {
    ...
    window.fetch = function(...args) {
        ...
        console.log('[Fetch Hook] start request:', {
            url: resource,
        ...
        return originalFetch.apply(this, args)
            .then(response => {
                console.log('[Fetch Hook] complete response:', {
                    url: resource,
                ...
                // content response with 克隆便查看
                const clonedResponse = response.clone();
                clonedResponse.text().then(text => {
                    console.log('[Fetch Hook] content response:', text);
                }).catch(err => {
                    console.log('[Fetch Hook] content response read 无法:', err);
                });
                
                return response;
            })
            .catch(error => {
                console.error('[Fetch Hook] failed request:', error);
                throw error;
            });
    };
    ...
    console.log('[Fetch Hook] Fetch API Hook 已激活');
            ...
            
            {
                ...
                description: "intercept call method all console，log output monitor page can of，包括console.log、, etc. console.error。",
...
(function() {
    ...
    console.log('[Console Hook] Console Methods Hook 已激活');
            ...
            
            {
                ...
                description: "settings monitor all operation read of and Cookie，trace page can of and 对Cookie访问修改。",
...
(function() {
    ...
    console.log('[Cookie Hook] Cookie Operations Hook 已激活');
            ...
            
            {
                ...
                description: "monitor all operation of LocalStorage，包括setItem、getItem、call method , etc. removeItem。",
...
(function() {
    ...
    localStorage.clear = function() {
        console.log('[LocalStorage Hook] clear: clear data all');
    ...
    console.log('[LocalStorage Hook] LocalStorage Hook 已激活');
            ...
            {
                name: "Hook WebSocket",
                description: "connection intercept and WebSocket消息传输，data monitor when 实通信。",
                code: `// Hook WebSocket
(function() {
    const originalWebSocket = window.WebSocket;
    
    window.WebSocket = function(url, protocols) {
        console.log('[WebSocket Hook] connection 创建:', url, protocols);
        
        const ws = new originalWebSocket(url, protocols);
        
        const originalSend = ws.send;
        ws.send = function(data) {
            console.log('[WebSocket Hook] send 消息:', data);
            return originalSend.apply(this, arguments);
        };
        
        ws.addEventListener('open', function(event) {
            console.log('[WebSocket Hook] open connection 已:', event);
        });
        
        ws.addEventListener('message', function(event) {
            console.log('[WebSocket Hook] to 收消息:', event.data);
        });
        
        ws.addEventListener('close', function(event) {
            console.log('[WebSocket Hook] closed connection:', event.code, event.reason);
        });
        
        ws.addEventListener('error', function(event) {
            console.log('[WebSocket Hook] error connection:', event);
        });
    ...
    console.log('[WebSocket Hook] WebSocket Hook 已激活');
            ...
            
            {
                ...
                description: "general call function Hook，monitor call function can of 指定情况。monitor variable function of 需要修改targetFunction指定要。",
                code: `// Hook Function Calls (template general)
(function() {
    // function of 修改这里指定要Hook
    const targetFunction = 'alert'; // 例如: 'setTimeout', 'setInterval', 'eval' , etc.
    ...
    if (window[targetFunction] && typeof window[targetFunction] === 'function') {
        ...
        window[targetFunction] = function(...args) {
            console.log('[Function Hook] call ' + targetFunction + ':', args);
            
            // get call 栈
            console.log('[Function Hook] call 栈:', new Error().stack);
        ...
        console.warn('[Function Hook] function 不存在:', targetFunction);
    }
})();`,
                isPreset: true,
                created: new Date().toISOString()
            ...
            
            {
                ...
                description: "add remove monitor event listen all of and 器，trace event page can of 绑定情况。",
...
(function() {
    ...
    EventTarget.prototype.addEventListener = function(type, listener, options) {
        console.log('[Event Hook] add event listen 器:', {
            target: this,
            type: type,
            listener: listener,
            options: options
    ...
    EventTarget.prototype.removeEventListener = function(type, listener, options) {
        console.log('[Event Hook] remove event listen 器:', {
            target: this,
            type: type,
            listener: listener,
            options: options
    ...
    console.log('[Event Hook] Event Listeners Hook 已激活');
            ...
            {
                name: 'Hook Eval',
                description: 'call listen of 网站对于eval',
                code: `(function () {
    ...
    window.eval.toString = function () {
        return evalCache.toString();
            ...
            {
                name: 'Hook Function',
                description: 'call function listen all of，console output print execute function all operation of of 都会在将要 JS 源码',
                code: `(function () {
...
`,
...

            {
                ...
                description: 'call listen of 对于JSON.parse',
                code: `(function () {
...
`,
    ...
    Math.random = function random() {
        ...
        return 12345689;
            ...
            {
                name: 'FakeCryptoAES',
                description: 'get parameters page(s) in of 网aes加密，如iv，, etc. key，console display in 在，console inject page in after 可在在输入customerFlag = true，debug display 启动，feature related time(s) of 再启动界面上与加密，call display digit(s) of 会加密所堆栈置，use customerEnc("content 加密")directly operation line(s) 可进加密，详细usemethod查看 item(s)目https://github.com/keecth/FakeCryptoJS?tab=readme-ov-file',
            ...
            {
                name: 'FakeCryptoRSA',
                description: 'get parameters page(s) in of 网aes加密，如iv，, etc. key，console display in 在，详细usemethod查看 item(s)目https://github.com/keecth/FakeCryptoJS?tab=readme-ov-file',
    ...
    // initialize script to 预设IndexedDB
    async initializePresets() {
        try {
            //console.log('[JSHook Presets] initialize start script 预设...');
            //console.log('[JSHook Presets] total count script 预设:', this.presetScripts.length);
            
            // check available no yes IndexedDBManager
            if (!window.IndexedDBManager) {
                console.error('[JSHook Presets] not found IndexedDBManager');
                return;
            }
            
            // get script has 现
            const existingScripts = await window.IndexedDBManager.loadJSScripts();
            //console.log('[JSHook Presets] quantity script has 现:', existingScripts.length);
            //console.log('[JSHook Presets] script has 现:', existingScripts.map(s => s.name));
            
            // check script no yes has 已经预设
            const hasPresets = existingScripts.some(script => script.isPreset === true);
            if (hasPresets) {
                //console.log('[JSHook Presets] script 预设已存在，initialize skip');
                return;
            }
            
            // add script as and when 预设标识间戳
            ...
            //console.log('[JSHook Presets] add quantity script 准备预设:', presetsWithMetadata.length);
            //console.log('[JSHook Presets] script name 预设:', presetsWithMetadata.map(s => s.name));
            
            // script script and has 合并预设现，save to after 然IndexedDB
            const allScripts = [...existingScripts, ...presetsWithMetadata];
            //console.log('[JSHook Presets] quantity script after 合并总:', allScripts.length);
            
            await window.IndexedDBManager.saveJSScripts(allScripts);
            
            //console.log('[JSHook Presets] initialize complete script 预设');
            
            // save results validate
            const verifyScripts = await window.IndexedDBManager.loadJSScripts();
            //console.log('[JSHook Presets] validate：save quantity script after:', verifyScripts.length);
            
        } catch (error) {
            console.error('[JSHook Presets] initialize failed script 预设:', error);
        }
    },
    
    // reset script 预设（debug on 发用）
    async resetPresets() {
        try {
            // delete script all 预设
            ...
            // save script 非预设回IndexedDB
            await window.IndexedDBManager.saveJSScripts(nonPresetScripts);
            
            console.log('[JSHook Presets] reset script 预设已');
            
        } catch (error) {
            console.error('[JSHook Presets] failed reset script 预设:', error);
        }
    }
};
...
// initialize complete script load auto page after 预设
