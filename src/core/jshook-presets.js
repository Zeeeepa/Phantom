// JSHook PresetScript库
const JSHookPresets = {
    // PresetScriptData
    presetScripts: [
            {
                name: "Hook XMLHttpRequest",
                description: "拦截And监控所有XMLHttpRequestRequest，Can查看RequestURL、Method、ParameterAnd响应Content。适Used for分析AJAXRequestAndAPI调用。",
...
(function() {
    ...
    console.log('[XHR Hook] XMLHttpRequest Hook Already激活');
            ...
            {
                name: "Hook Fetch API",
                description: "拦截And监控所有fetchRequest，DisplayRequestAnd响应的详细Information。现代浏览器中替代XMLHttpRequest的新API监控。",
                code: `// Hook Fetch API
(function() {
        ...
        return originalFetch.apply(this, args)
            .then(response => {
                ...
                clonedResponse.text().then(text => {
                    console.log('[Fetch Hook] 响应Content:', text);
                }).catch(err => {
                    console.log('[Fetch Hook] None法Read响应Content:', err);
                });
    ...
    console.log('[Fetch Hook] Fetch API Hook Already激活');
            ...
            
            {
                ...
                description: "拦截所有consoleMethod调用，Can监控Page的日志Output，包括console.log、console.error等。",
...
(function() {
    ...
    console.log('[Console Hook] Console Methods Hook Already激活');
            ...
            
            {
                ...
                description: "监控所有Cookie的ReadAndSettings操作，Can追踪Page对Cookie的访问And修改。",
...
(function() {
    ...
    console.log('[Cookie Hook] Cookie Operations Hook Already激活');
            ...
            
            {
                ...
                description: "监控LocalStorage的所有操作，包括setItem、getItem、removeItem等Method调用。",
...
(function() {
    ...
    localStorage.clear = function() {
        console.log('[LocalStorage Hook] clear: Clear所有Data');
    ...
    console.log('[LocalStorage Hook] LocalStorage Hook Already激活');
            ...
            {
                name: "Hook WebSocket",
                description: "拦截WebSocketConnectionAnd消息传输，监控实时通信Data。",
                code: `// Hook WebSocket
(function() {
    const originalWebSocket = window.WebSocket;
    
    window.WebSocket = function(url, protocols) {
        console.log('[WebSocket Hook] CreateConnection:', url, protocols);
        
        const ws = new originalWebSocket(url, protocols);
        
        const originalSend = ws.send;
        ws.send = function(data) {
            console.log('[WebSocket Hook] Send消息:', data);
            return originalSend.apply(this, arguments);
        };
        
        ws.addEventListener('open', function(event) {
            console.log('[WebSocket Hook] ConnectionAlready打开:', event);
        });
        ...
        ws.addEventListener('close', function(event) {
            console.log('[WebSocket Hook] ConnectionAlreadyClose:', event.code, event.reason);
        });
        
        ws.addEventListener('error', function(event) {
            console.log('[WebSocket Hook] ConnectionError:', event);
        });
    ...
    console.log('[WebSocket Hook] WebSocket Hook Already激活');
            ...
            
            {
                ...
                description: "通用Function调用Hook，Can监控指定Function的调用情况。Need修改targetFunctionVariable指定要监控的Function。",
                code: `// Hook Function Calls (通用模板)
(function() {
    // 修改Here指定要Hook的Function
    ...
    if (window[targetFunction] && typeof window[targetFunction] === 'function') {
        ...
        console.warn('[Function Hook] Function不存在:', targetFunction);
    }
})();`,
                isPreset: true,
                created: new Date().toISOString()
            ...
            
            {
                ...
                description: "监控所有事件Listen器的AddAndRemove，Can追踪Page的事件绑定情况。",
...
(function() {
    ...
    EventTarget.prototype.addEventListener = function(type, listener, options) {
        console.log('[Event Hook] Add事件Listen器:', {
            target: this,
            type: type,
            listener: listener,
            options: options
    ...
    EventTarget.prototype.removeEventListener = function(type, listener, options) {
        console.log('[Event Hook] Remove事件Listen器:', {
            target: this,
            type: type,
            listener: listener,
            options: options
    ...
    window.eval.toString = function () {
        return evalCache.toString();
            ...
            {
                name: 'Hook Function',
                description: 'Listen所有Function的调用，所有的Function操作都会在控制台打印Output将要Execute的 JS Source码',
                code: `(function () {
...
`,
...

            {
                ...
                description: 'Listen对于JSON.parse的调用',
                code: `(function () {
...
`,
    ...
    Math.random = function random() {
        ...
        return 12345689;
            ...
            {
                name: 'FakeCryptoAES',
                description: 'Get网页中的aesEncryptionParameter，如iv，key等，在控制台中Display，可在注入After在Page控制台中InputcustomerFlag = true，Start调试Display，再次Start界面上与EncryptionRelated的功能，会DisplayEncryption所调用的堆栈位置，使用customerEnc("EncryptionContent")可DirectPerformEncryption操作，详细使用Method查看Projecthttps://github.com/keecth/FakeCryptoJS?tab=readme-ov-file',
            ...
            {
                name: 'FakeCryptoRSA',
                description: 'Get网页中的aesEncryptionParameter，如iv，key等，在控制台中Display，详细使用Method查看Projecthttps://github.com/keecth/FakeCryptoJS?tab=readme-ov-file',
    ...
    // InitializePresetScript到IndexedDB
    async initializePresets() {
        try {
            //console.log('[JSHook Presets] StartInitializePresetScript...');
            //console.log('[JSHook Presets] PresetScript总数:', this.presetScripts.length);
            
            // CheckIndexedDBManager是否Available
            if (!window.IndexedDBManager) {
                console.error('[JSHook Presets] IndexedDBManagerNot found');
                return;
            }
            
            // Get现有Script
            const existingScripts = await window.IndexedDBManager.loadJSScripts();
            //console.log('[JSHook Presets] 现有Script数量:', existingScripts.length);
            //console.log('[JSHook Presets] 现有Script:', existingScripts.map(s => s.name));
            
            // Check是否Already经有PresetScript
            const hasPresets = existingScripts.some(script => script.isPreset === true);
            if (hasPresets) {
                //console.log('[JSHook Presets] PresetScriptAlready存在，跳过Initialize');
                return;
            }
            
            // 为PresetScriptAdd标识AndTime戳
            ...
            //console.log('[JSHook Presets] PrepareAddPresetScript数量:', presetsWithMetadata.length);
            //console.log('[JSHook Presets] PresetScript名称:', presetsWithMetadata.map(s => s.name));
            
            // 合AndPresetScriptAnd现有Script，ThenSave to IndexedDB
            const allScripts = [...existingScripts, ...presetsWithMetadata];
            //console.log('[JSHook Presets] 合AndAfter总Script数量:', allScripts.length);
            
            await window.IndexedDBManager.saveJSScripts(allScripts);
            
            //console.log('[JSHook Presets] PresetScriptInitializeComplete');
            
            // ValidateSave results
            const verifyScripts = await window.IndexedDBManager.loadJSScripts();
            //console.log('[JSHook Presets] Validate：SaveAfterScript数量:', verifyScripts.length);
            
        } catch (error) {
            console.error('[JSHook Presets] InitializePresetScriptFailed:', error);
        }
    },
    
    // ResetPresetScript（开发调试用）
    async resetPresets() {
        try {
            // Delete所有PresetScript
            ...
            // Save非PresetScript回IndexedDB
            await window.IndexedDBManager.saveJSScripts(nonPresetScripts);
            
            console.log('[JSHook Presets] PresetScriptAlreadyReset');
            
        } catch (error) {
            console.error('[JSHook Presets] ResetPresetScriptFailed:', error);
        }
    }
};
...
// PageLoading completeAfterAutoInitializePresetScript
