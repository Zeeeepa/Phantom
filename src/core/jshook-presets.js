// JSHook 预设 script library
const JSHookPresets = {
    // 预设 script data
    presetScripts: [
            {
                name: "Hook XMLHttpRequest",
                description: "拦截并监控allXMLHttpRequest请求，可以查看 request URL、method、parameter and response content。适for analysis AJAX request andAPI调用。",
...
(function() {
    ...
    console.log('[XHR Hook] XMLHttpRequest Hook already激活');
            ...
            {
                name: "Hook Fetch API",
                description: "intercept 并 monitor allfetch request，display request and response   detailed information。现代浏览器in替代XMLHttpRequest 新API监控。",
                code: `// Hook Fetch API
(function() {
    ...
    window.fetch = function(...args) {
        ...
        return originalFetch.apply(this, args)
            .then(response => {
                ...
                // 克隆 response 以便查看 content
                const clonedResponse = response.clone();
                clonedResponse.text().then(text => {
                    console.log('[Fetch Hook] response content:', text);
                }).catch(err => {
                    console.log('[Fetch Hook] 无法read response content:', err);
                });
    ...
    console.log('[Fetch Hook] Fetch API Hook already激活');
            ...
            
            {
                ...
                description: "intercept allconsole method 调用，可以 monitor page   log 输出，package 括console.log、console.error等。",
...
(function() {
    ...
    console.log('[Console Hook] Console Methods Hook already激活');
            ...
            
            {
                ...
                description: "monitor allCookie readand settings 操作，可以 trace page 对Cookie 访问and modify。",
...
(function() {
    ...
    console.log('[Cookie Hook] Cookie Operations Hook already激活');
            ...
            
            {
                ...
                description: "monitor LocalStorage all操作，package 括setItem、getItem、removeItem等 method 调用。",
...
(function() {
    ...
    console.log('[LocalStorage Hook] LocalStorage Hook already激活');
            ...
            {
                name: "Hook WebSocket",
                description: "intercept WebSocket连接and message 传输，monitor 实时通信 data。",
                code: `// Hook WebSocket
(function() {
    ...
    window.WebSocket = function(url, protocols) {
        ...
        ws.send = function(data) {
            console.log('[WebSocket Hook] 发送 message:', data);
            return originalSend.apply(this, arguments);
        };
        
        ws.addEventListener('open', function(event) {
            console.log('[WebSocket Hook] 连接already open:', event);
        });
        
        ws.addEventListener('message', function(event) {
            console.log('[WebSocket Hook] received message:', event.data);
        });
        
        ws.addEventListener('close', function(event) {
            console.log('[WebSocket Hook] 连接already close:', event.code, event.reason);
        });
        
        ws.addEventListener('error', function(event) {
            console.log('[WebSocket Hook] 连接 error:', event);
        });
    ...
    console.log('[WebSocket Hook] WebSocket Hook already激活');
            ...
            
            {
                ...
                description: "general function 调用Hook，可以 monitor 指定 function  调用情况。require modify targetFunction variable 指定要 monitor   function。",
                code: `// Hook Function Calls (general template)
(function() {
    // modify 这里指定要Hook  function
    ...
    if (window[targetFunction] && typeof window[targetFunction] === 'function') {
        ...
        window[targetFunction] = function(...args) {
            ...
            // 获取调用 stack
            console.log('[Function Hook] 调用 stack:', new Error().stack);
        ...
        console.warn('[Function Hook] function do not存in:', targetFunction);
    }
})();`,
                isPreset: true,
                created: new Date().toISOString()
            ...
            
            {
                ...
                description: "monitor all event listener   add and remove，可以 trace page   event 绑定情况。",
...
(function() {
    ...
    EventTarget.prototype.addEventListener = function(type, listener, options) {
        console.log('[Event Hook] add event listener:', {
            target: this,
            type: type,
            listener: listener,
            options: options
    ...
    EventTarget.prototype.removeEventListener = function(type, listener, options) {
        console.log('[Event Hook] remove event listener:', {
            target: this,
            type: type,
            listener: listener,
            options: options
    ...
    console.log('[Event Hook] Event Listeners Hook already激活');
            ...
            {
                name: 'Hook Eval',
                description: 'listenweb站对于eval 调用',
                code: `(function () {
    ...
    window.eval.toString = function () {
        return evalCache.toString();
            ...
            {
                name: 'Hook Function',
                description: 'listenall function  调用，all  function 操作都会in控制台打印输出将要 execute   JS 源码',
                code: `(function () {
...
`,
...

            {
                ...
                description: 'listen对于JSON.parse 调用',
                code: `(function () {
...
`,
            ...
            {
                name: '固定随机数  value',
                description: 'modify 随机数  value to固定 value',
                code: `(function () {
    ...
    Math.random = function random() {
        ...
        return 12345689;
            ...
            {
                name: 'FakeCryptoAES',
                description: '获取web页in aes encryption parameter，如iv，key等，in控制台in display，可in inject 后in page 控制台in输入customerFlag = true，启动 debug display，再次启动界面上与 encryption 相关  feature，会 display encryption 所调用 堆 stack 位置，usecustomerEnc("encryption content")可directly进行 encryption 操作，详细usemethod查看项目https://github.com/keecth/FakeCryptoJS?tab=readme-ov-file',
            ...
            {
                name: 'FakeCryptoRSA',
                description: '获取web页in aes encryption parameter，如iv，key等，in控制台in display，详细usemethod查看项目https://github.com/keecth/FakeCryptoJS?tab=readme-ov-file',
    ...
    // initialize 预设 script 到IndexedDB
    async initializePresets() {
        try {
            //console.log('[JSHook Presets] start initialize 预设 script ...');
            //console.log('[JSHook Presets] 预设 script total:', this.presetScripts.length);
            ...
            //console.log('[JSHook Presets] 现有 script count:', existingScripts.length);
            //console.log('[JSHook Presets] 现有 script:', existingScripts.map(s => s.name));
            
            // check 是否already经有预设 script
            const hasPresets = existingScripts.some(script => script.isPreset === true);
            if (hasPresets) {
                //console.log('[JSHook Presets] 预设 script already存in，skip initialize');
                return;
            }
            
            // to预设 script add 标识and时间戳
            ...
            //console.log('[JSHook Presets] 准备 add 预设 script count:', presetsWithMetadata.length);
            //console.log('[JSHook Presets] 预设 script 名称:', presetsWithMetadata.map(s => s.name));
            
            // 合并预设 script and现有 script，然后 save 到IndexedDB
            const allScripts = [...existingScripts, ...presetsWithMetadata];
            //console.log('[JSHook Presets] 合并后总 script count:', allScripts.length);
            
            await window.IndexedDBManager.saveJSScripts(allScripts);
            
            //console.log('[JSHook Presets] 预设 script initialize complete');
            
            // validate save result
            const verifyScripts = await window.IndexedDBManager.loadJSScripts();
            //console.log('[JSHook Presets] validate：save 后 script count:', verifyScripts.length);
            
        } catch (error) {
            console.error('[JSHook Presets] initialize 预设 script failed:', error);
        }
    },
    
    // 重置预设 script（开发 debug 用）
    async resetPresets() {
        try {
            // delete all预设 script
            ...
            // save 非预设 script 回IndexedDB
            await window.IndexedDBManager.saveJSScripts(nonPresetScripts);
            
            console.log('[JSHook Presets] 预设 script already重置');
            
        } catch (error) {
            console.error('[JSHook Presets] 重置预设 script failed:', error);
        }
    }
};
...
// page load complete 后 automatic initialize 预设 script
